...
import SettingsInstallApk from "@/components/settings/install_apk";

type SettingsSectionType = "scrcpy" | "device" | "install_apk";
...
const SettingsList: {
    ...
    },
    {
        id: "install_apk",
        label: "install_apk",
        description: "install_apk_settings_description",
        icon: PiAndroidLogoDuotone,
        notGlobal: false,
        component: (config, setConfig) => <SettingsInstallApk config={config} setConfig={setConfig} />,
    },
];
                ...
                <div className={cls.sidebar}>
                        ...
                        <Select.Root
                            ...
                            onValueChange={(value) => {
                                if (value === "global") {
                                    ...
                                    setSelectedDevice({
                                        type: value.startsWith("w:") ? "websocket" : "usb",
                                        id: value.replace("w:", "").replace("u:", ""),
                                    });
                                }
                            }}
                            value={
                                selectedDevice
                                    ? selectedDevice.type === "websocket"
                                        ? "w:" + selectedDevice.id
                                        : "u:" + selectedDevice.id
                                    : "global"
                            }
                        >
                            ...
                            <Select.Content variant="soft">
                                ...
                                <Select.Group>
                                    ...
                                    {tabsWebsocket.length > 0 ? (
                                        tabsWebsocket.map((tab) => (
                    ...
                    <Flex direction="column" gap="2" style={{ width: "100%" }}>
                        {SettingsList.map((item) =>
                            !selectedDevice && item.notGlobal ? null : (
                                <Button
                                    key={item.id}
                                    variant="ghost"
                    ...
                    <div className={cls.content}>
                        {selectedSectionContent && (
                            <>
                                <Flex direction="column" gap="1" style={{ width: "100%" }}>
                                    <Text size="3" weight="bold">
                                        {t(selectedSectionContent.label)}
                                    </Text>
                                    <Text size="1" color="gray">
                                        {t(selectedSectionContent.description)}
                                    </Text>
                                </Flex>
