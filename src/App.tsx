...
function App() {
    ...
    const tabClose = (id: string) => {
        ...
        setTabs(
            filteredTabs.map((tab, i) => ({ ...tab, active: i === newIndex })),
        );
    };
    ...
    useEffect(() => {
        ...
        content
            ...
            .map((c) => {
                const device = listDevices.find(
                    (d) => d.raw.manufacturerName + d.name + d.serial === c.id,
                );
                if (!device) {
                    tabClose(c.id);
        ...
        if (deviceDisconnected.length > 0)
            toast.error(
                t("device_disconnected_description") +
                    " " +
                    deviceDisconnected.join(", "),
            );
                ...
                <div className={cls.StackContainer} key={index}>
                    <Container
                        ...
                        shouldShowWelcome={
                            ...
                            (index > 0 &&
                                content.find((c) => c.stackNo === index) ===
                                    undefined &&
                                content.find((c) => c.stackNo === index - 1) !==
                                    undefined)
                        }
                        stackController={
